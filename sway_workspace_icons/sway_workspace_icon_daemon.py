...
def get_resource_path(filename: str) -> Path:
    ...
    resource_path = module_dir / filename

    # If the resource exists in the module directory, use it
...
class WorkspaceIconDaemon:
    ...
    def get_programs_by_workspace(
        ...
        for workspace in sway.get_tree().workspaces():
            ...
            programs = [
                ...
                if (program_name := WorkspaceIconDaemon._get_window_name(window))
                and program_name not in ignored_programs
            ]
    ...
    def install_icon_font(font_output_path: Path) -> None:
        ...
        Raises:
            FileNotFoundError: If font_output_path doesn't exist.
            OSError: If font installation fails.
        """
        if not font_output_path.exists():
            raise FileNotFoundError(f"Font file does not exist: {font_output_path}")
    ...
    def process_new_programs(self) -> bool:
        ...
        self.install_icon_font(self.font_output_path)
        return True
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "numbers_subscript: show one icon per program with count in subscript (default); "
...
def main() -> None:
    ...
    if args.rebuild:
        ...
        if rebuilt:
            daemon.install_icon_font(daemon.font_output_path)
